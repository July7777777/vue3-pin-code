...
<template>
  <div class="pin-code" :id="id" :class="[shape]">
    <input
      v-for="(_, index) in maxInputs"
      :class="[
        'pin-code-input',
        isMask ? '' : 'font',
        pinCode[index].length === 0 ? 'emptyBGC' : '',
      ]"
      :key="index"
      type="text"
      maxlength="1"
...
import { toRefs, ref, onMounted, watchEffect,defineProps } from "vue";
import { PinCodeProps, PinCodePropsType } from "./props";
// const props = defineProps(Props);
const props = defineProps(PinCodeProps);
const { maxInputs, completeFn, shape, isMask, maskFlag, id, isPaste } = toRefs(
  props
) as any;
const InputElementArr = ref<readonly HTMLInputElement[]>([]);
const pinCode = ref<string[]>(Array(props.maxInputs).fill(""));

// 如果isPaste是来自props或其它响应式数据，请确保它是响应式的

// 使用watchEffect来监听isPaste或其它可能影响idElement的变化
watchEffect(() => {
...
const pasteHandler = (event: any) => {
  ...
  pinCode.value.every((d) => Boolean(d)) &&
